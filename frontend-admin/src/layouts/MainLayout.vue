  ...
  <div class="flex h-screen">
    ...
    <aside class="w-64 bg-dark-900 border-r border-dark-800 flex flex-col">
      ...
      <div class="p-6 border-b border-dark-800">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-xs text-dark-400">行情监控系统</p>
          </div>
        </div>
      </div>
      ...
      <div class="p-4 border-t border-dark-800">
        ...
        <button
          @click="handleLogout"
          class="w-full px-4 py-2 text-sm text-dark-300 hover:text-white hover:bg-dark-800 rounded-lg transition-colors"
        >
          退出登录
        </button>
      </div>
...
const menuItems = computed(() => {
  const items = [
    ...
    { path: '/history', name: '事件中心', icon: HistoryIcon }
  ]
  
  if (authStore.isAdmin) {
    items.push({ path: '/users', name: '用户管理', icon: UsersIcon })
  }
